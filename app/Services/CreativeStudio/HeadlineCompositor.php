...
class HeadlineCompositor
{
    ...
    public function pegarBannerInferior($im, int $w, int $h, string $texto, string $colorCinta = self::AMARILLO): void
    {
        ...
        $this->dibujarCinta($im, $cx, $cy, $cintaW, $cintaH, -4, $colorCinta, $texto, $size, $font);
    }
    ...
    public function pegarStickerCompleto($im, int $w, int $h, string $headline, ?string $precio = null, ?string $specs = null): void
    {
        ...
        $this->dibujarCinta($im, $cx, $cyHead, $cintaHeadW, $cintaHeadH, -4, self::FUCSIA, $headline, $sizeHead, $font);
        ...
        if ($precio) {
            ...
            $cyPrecio = $cyHead + $cintaHeadH * 0.55 + $cintaPrecioH * 0.5;
            $this->dibujarCinta($im, $cx, $cyPrecio, $cintaPrecioW, $cintaPrecioH, -2, self::AMARILLO, $precio, $sizePrecio, $font);

            if ($specs) {
                $sizeSpecs = $w * 0.032;
                $anchoSpecs = $this->anchoTexto(mb_strtoupper($specs), $font, $sizeSpecs);
                [$nr, $ng, $nb] = $this->hexToRgb(self::NAVY);
                $navy = imagecolorallocate($im, $nr, $ng, $nb);
                $blanco = imagecolorallocate($im, 255, 255, 255);
                $padX = $sizeSpecs * 1.2;
                $barraY = (int) ($cyPrecio + $cintaPrecioH * 0.55);
                imagefilledrectangle($im, (int) ($cx - $anchoSpecs / 2 - $padX), $barraY, (int) ($cx + $anchoSpecs / 2 + $padX), (int) ($barraY + $sizeSpecs * 1.8), $navy);
                imagettftext($im, $sizeSpecs, 0, (int) ($cx - $anchoSpecs / 2), (int) ($barraY + $sizeSpecs * 1.35), $blanco, $font, mb_strtoupper($specs));
            }
        }
    }

    /** Cinta rotada con banderines en las puntas y texto con contorno — el mismo lenguaje visual de la marca. */
    protected function dibujarCinta($destino, float $cx, float $cy, float $cintaW, float $cintaH, float $anguloGrados, string $colorHex, string $texto, float $size, string $font): void
    {
        $margen = $cintaH * 1.4;
        ...
        [$nr, $ng, $nb] = $this->hexToRgb(self::NAVY);
        $navy = imagecolorallocate($tmp, $nr, $ng, $nb);
        ...
        imagepolygon($tmp, $puntos, $navy);

        $blanco = imagecolorallocate($tmp, 255, 255, 255);
        $tx = $margen + $cintaW / 2;
        $ty = $margen + $cintaH / 2 + $size * 0.34;
        $this->textoConContorno($tmp, $texto, (int) $tx, (int) $ty, $size, $font, $blanco, $navy, max(2, (int) ($size * 0.08)));
