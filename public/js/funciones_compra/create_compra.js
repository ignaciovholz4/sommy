...
if (addArticuloBtn) {
    addArticuloBtn.addEventListener("click", () => {
        ...
        let existingRow = compraItems.querySelector(
        ...
        fetch(`/price-lists/applicable/${id}?context=purchase`)
            .then(res => res.json())
            .then(lists => {
        ...
        if (ivaSelect) ivaSelect.value = ivaDefault;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (sucursalSelect) {
        sucursalSelect.addEventListener("change", function() {
            ...
            fetch(`/sucursal/${sucursalId}/articulos-disponibles?context=compra`)
                .then(res => res.json())
                .then(data => {
                    ...
                    data.articulos.forEach(a => {
                        ...
                        opt.dataset.precio = a.pcompra_con_iva;
                        opt.dataset.iva = a.iva_compra;
                    ...
                    data.combinaciones.forEach(c => {
                        ...
                        opt.dataset.tipo = "2";
                        opt.dataset.precio = c.pcompra_con_iva;
                        opt.dataset.iva = c.iva_compra;
